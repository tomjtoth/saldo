...
it("populating DB works as expected", () => {
  ...
  cy.readDb().then(({ baseline, response }) => {
    ...
    expect(baseline[0].groups[0].consumption).to.deep.eq(
      baseline[1].groups[0].consumption
    );
    expect(baseline[0].groups[0].consumption).to.deep.eq(
      baseline[2].groups[0].consumption
    );
    expect(baseline[1].groups[1].consumption).to.deep.eq(
    ...
    const u1g1withExtendedMinMaxes = {
      ...baseline[0].groups[0].balance,
      minMaxes: baseline[1].groups[0].balance.minMaxes,
    };

    expect(u1g1withExtendedMinMaxes).to.deep.eq(baseline[1].groups[0].balance);
    expect(u1g1withExtendedMinMaxes).to.deep.eq(baseline[2].groups[0].balance);
    expect(baseline[1].groups[1].balance).to.deep.eq(
