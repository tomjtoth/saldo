...
describe("datetimeFunctions", () => {
  it("convert date '2020-01-01' properly to 0", () => {
    const base = VDate.toInt("2020-01-01");
    expect(base).toEqual(0);
  });

  it("convert date '2020-01-02' properly to 1", () => {
    const base = VDate.toInt("2020-01-02");
  ...
  it("convert datetime '2020-01-02' properly to 24*60*60", () => {
    const jan2 = DateTime.fromFormat("2020-01-02", "y-M-d", DATETIME_OPTIONS);
    const diff = VDate.timeToInt(jan2);

    expect(diff).toEqual(1 * 24 * 60 * 60);
  });

  it("convert 31*24*60*60 properly to datetime '2020-02-01'", () => {
